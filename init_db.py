...
KNOWLEDGE = [
    ...
    ("action", 1, "動作性（高）",
     "ある瞬間に変化や刺激が発生する → 継続性（ー）", "act_high.svg"),
    ("action", 2, "動作性（中）",
     "感情・感覚・状態を表すが、時間の継続がある → 継続性（＋）、変化結果の継続性（＋）", "act_mid.svg"),
    ("action", 3, "動作性（低）",
     "動作性が低いもの（だらだら・のんびり など）", "act_low.svg"),
    ("action", 4, "動作性（ー）",
     "動作性がないもの", None),

    ("volition", 1, "意志性（＋）",
     "動作主が自分の意志で行うことができる動き・行為を表す。", "vol_plus.svg"),
    ("volition", 2, "意志性（ー）",
     "自分の意志で起こすことができない感情・感覚・状態を表す。", "vol_minus.svg"),
    ("volition", 3, "意志性（関与しない）",
     "状況に反応して起こる動きで、意志で始めるとも言えないもの（おろおろ・あたふた など）。", "vol_na.svg"),
...
# ---------------------------------------------------------------------
#  出題対象：8カテゴリー × 2語 = 16語
#    研究データは39語すべて残したうえで、学習に使う語だけをここで選ぶ。
#    選定の基準：
#      ・音韻形態5型をすべて含む（反復形／変種／っ＋り／ん＋り／っ終わり）
#      ・音の形からの予測が外れる語を含む（ほっ・ゆっくり・ぎくしゃく）
#      ・対比ペア（いらいら／いらっ など）が成立する組を残す
# ---------------------------------------------------------------------
SELECTED_WORD_IDS = [
    "iraira", "zukizuki",        # 感情・感覚（継続性があるもの）
    "bikkuri", "hotto",          # 感情・感覚（変化結果の継続性があるもの）
    "iratto", "zukitto",         # 感情・感覚（継続性がないもの）
    "burabura", "urouro",        # 動作性が高いもの（意志性があるもの）
    "kyorokyoro", "orooro",      # 動作性が高いもの（意志性が関わらないもの）
    "nikoniko", "gikushaku",     # 動作性が中程度のもの
    "gorogoro", "yukkuri",       # 動作性が低いもの（意志性があるもの）
    "daradara", "bonyari",       # 動作性が低いもの（意志性がないもの）
]

WORDS = [w for w in WORDS_ALL if w[0] in SELECTED_WORD_IDS]


# =====================================================================
#  6. Step4：語ごとの例文（2問ずつ。出題対象の語のぶんだけ使う）
...
CONTRAST_PAIRS = [
    ...
    ("continuity", 2, "zukizuki", "bikkuri", "continuity",
     "どちらも感情・感覚ですが、「ずきずき」は痛みそのものがくり返し続き、「びっくり」は驚く変化が一度起きて、その結果が残っているだけです。"),
    ...
    ("volition", 3, "urouro", "zukizuki", "volition",
     "自分の意志で始められる動き（うろうろ）と、意志では起こせない感覚（ずきずき）の対比です。"),
...
WORKED_EXAMPLES = [
    ...
    ("continuity", "bikkuri", "変化結果の＋（変化結果の継続性）",
     "「一度きりの変化」と「そのあと残る状態」を分けて考えるのがコツです。驚いた状態が続いていても、＋継続性ではありません。"),
    ("action", "gikushaku", "動作性（中）",
     "音の形（2モーラ反復形の変種）からは動作性（高）と予測されますが、この語は例外です。最後は意味で決めます。"),
    ("volition", "orooro", "意志性は関与しない",
...
WORKED_STEPS = [
    ...
    ("continuity", 1, "① 場面を思い浮かべる：どんなときに「びっくり」する？",
     "後ろから急に声をかけられた、その瞬間に起こります。"),
    ("continuity", 2, "② 今もまだ「びっくりする」という変化が続いている？",
     "いいえ。驚くという変化そのものは、声をかけられた一度きりです。"),
    ("continuity", 3, "③ では、変化の結果は残っている？",
     "残っています。驚いた状態はそのあとも続きます。だから〈変化結果の＋〉です。"),

    ("action", 1, "① まず継続性は？",
     "＋継続性です。かみ合わない状態が続いています（一瞬ではありません）。"),
    ("action", 2, "② 外から見える具体的で激しい動きはある？",
     "ありません。動きや関係が「なめらかでない」ようすを表しています。"),
    ("action", 3, "③ 動きより、ようす・状態が中心か？",
     "そうです。動きはありますが激しくないので〈動作性（中）〉になります。"),

    ("volition", 1, "①「今からおろおろしよう」と言える？",
     "言えません。だから ＋意志性ではありません。"),
    ("volition", 2, "② 感情・感覚か、それとも動き・ようすか？",
     "うろたえて落ち着かない「ようす」です。心の中だけの話ではありません。"),
    ("volition", 3, "③ なぜ起こっている？",
     "突然のトラブルという状況に反応して起こっています。意志が関与しないタイプです。"),
...
PRACTICE = [q for q in PRACTICE_ALL if q[0] in SELECTED_WORD_IDS]
...
def verify():
    ...
    sel_by_cat = {}
    for w in WORDS:
        sel_by_cat.setdefault(w[4], []).append(w[1])
    for c in CATEGORIES:
        n = len(sel_by_cat.get(c[0], []))
        if n != 2:
            errors.append(f'[選定] {c[2]} の出題語が {n} 語です（各カテゴリー2語）')
    if len(WORDS) != 16:
        errors.append(f"[選定] 出題語が {len(WORDS)} 語です（16語）")
    unknown = [i for i in SELECTED_WORD_IDS if i not in {w[0] for w in WORDS_ALL}]
    if unknown:
        errors.append(f"[選定] 存在しない語が選ばれています：{unknown}")
    # 音韻形態5型がすべて含まれること
    sel_phons = {w[3] for w in WORDS}
    for pf in PHON_FORMS:
        if pf[0] not in sel_phons:
            errors.append(f'[選定] 音韻形態「{pf[2]}」の語が1つも含まれていません')
    # 予測が外れる語（例外）が含まれること
    phon_pred = {pf[0]: {"continuity": pf[6], "action": pf[7]} for pf in PHON_FORMS}
    cat_feat_sel = {c[0]: {"continuity": c[4], "action": c[5]} for c in CATEGORIES}
    n_exc = 0
    for w in WORDS:
        for f in ("continuity", "action"):
            pr = phon_pred[w[3]][f]
            if pr and cat_feat_sel[w[4]][f] != pr:
                n_exc += 1
                break
    if n_exc < 2:
        errors.append(f"[選定] 音の形からの予測が外れる語が {n_exc} 語しかありません")
    ...
    for st, wid, concl, note in WORKED_EXAMPLES:
        ...
        elif wid not in SELECTED_WORD_IDS:
            errors.append(f"[学習] 思考ガイドが出題対象外の語を参照しています：{wid}")
    for st, o, wa, wb, feat, point in CONTRAST_PAIRS:
        for wid in (wa, wb):
            if wid not in SELECTED_WORD_IDS:
                errors.append(f"[学習] 対比ペアが出題対象外の語を参照しています：{wid}")
