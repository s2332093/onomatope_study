...
PRACTICE = {
    ...
    "emo_moment": {
        "frame": "その瞬間、{word}[ ？ ]。",
        "explain": "ー継続性（一瞬で成立）なので「としてしまった」が自然です。「としている」（状態の継続）や「としつづけた」は使えません。",
        "options": [
            ("としてしまった", "瞬間の完了",   1),
            ("としている",     "状態の継続",   0),
            ("としつづけた",   "継続",         0),
    ...
    "act_high_vol": {
        ...
        "options": [
            ("するところだ", "これから行う直前（意志的）",   1),
            ("してきた",     "状態の出現",                   0),
            ("したところだ", "変化の完了直後",               0),
        ],
    },
    "act_high_novol": {
        "frame": "道に迷って、あちこち{word}[ ？ ]。",
...
def main():
    ...
    for cat_id, p in PRACTICE.items():
        cur.execute("INSERT INTO practice VALUES (?,?,?)", (cat_id, p["frame"], p["explain"]))
        cur.executemany("INSERT INTO practice_options VALUES (?,?,?,?,?)",
                        [(cat_id, i, label, meaning, correct)
                         for i, (label, meaning, correct) in enumerate(p["options"])])
